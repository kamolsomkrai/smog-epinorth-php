...
if (!Yii::$app->request->get('month')) {
    for ($i = 1; $i <= 53; $i++) {
        $items[] = ['attribute' => "wk$i", 'hAlign' => GridView::ALIGN_CENTER, 'class' => '\kartik\grid\BooleanColumn'];
    }
} else {
    $dayTotal = cal_days_in_month(CAL_GREGORIAN, Yii::$app->request->get('month'), Yii::$app->request->get('year') - 543);
    for ($i = 1; $i <= $dayTotal; $i++) {
        $items[] = ['attribute' => "d$i", 'header' => "$i", 'hAlign' => GridView::ALIGN_CENTER, 'class' => '\kartik\grid\BooleanColumn'];
    }
}
?>
